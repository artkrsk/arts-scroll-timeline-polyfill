import type {
  ...
  PolyfillUnitValue,
  PolyfillAnimation,
  PolyfillAnimationEffect,
  PolyfillComputedTiming,
  PolyfillAnimationOptions,
  PolyfillRangeInput,
  TimelineRangeName,
} from '../public/index.js'
import type { AnimationState, NativeTiming } from './animation-types.js'
import type { AnimationRelation } from './parser-types.js'
import type { AnimationRange, ViewRangeOffset, ResolvedRangePart } from './timeline-types.js'
...
import { splitIntoComponentValues } from './utils.js'
import { simplifyCalculation } from './simplify-calculation.js'

export const nativeDocumentGetAnimations = document.getAnimations
export const nativeElementGetAnimations = window.Element.prototype.getAnimations
const nativeElementAnimate = window.Element.prototype.animate
const nativeAnimation = window.Animation

export class PromiseWrapper<T> {
  state: 'pending' | 'resolved' | 'rejected' = 'pending'
  private nativeResolve: ((value: T | PromiseLike<T>) => void) | null = null
  private nativeReject: ((reason?: unknown) => void) | null = null
  ...
  resolve(value: T): void {
    this.state = 'resolved'
    required(this.nativeResolve)(value)
  }
  reject(reason: unknown): void {
    this.state = 'rejected'
    void this.promise.catch(() => {})
    required(this.nativeReject)(reason)
  }
}

function createReadyPromise(details: AnimationState): void {
  details.readyPromise = new PromiseWrapper<ProxyAnimation>()
  ...
  requestAnimationFrame(() => {
    ...
    if (
      ...
      (details.startTime !== null || details.holdTime !== null)
    ) {
...
function toCssNumberish(
  ...
  return numeric.CSS.percent(percent)
}
...
function fromCssNumberish(details: AnimationState, value: PolyfillNumberish | null): number | null {
  if (!details.timeline) {
    // Document timeline
    if (value == null || typeof value === 'number') return value

    const convertedTime = value.to?.('ms')
    if (convertedTime) return convertedTime.value

    throw new DOMException(
      'numeric.CSSNumericValue must be either a number or a time value for ' +
    ...
    if (value instanceof numeric.CSSUnitValue && value.unit === 'percent') {
      const rangeDuration = details.rangeDuration ?? 100
      const duration = effectEnd(details)
      return rangeDuration ? (value.value * duration) / rangeDuration : 0
    }

    throw new DOMException(
      'numeric.CSSNumericValue must be a percentage for progress based animations.',
      'NotSupportedError',
    )
  }
}

function normalizedTiming(details: AnimationState): NativeTiming {
  ...
  const timing = readTiming(details.proxy.effect)
  return details.normalizedTiming || timing
}

function commitPendingPlay(details: AnimationState): void {
  ...
  if (details.holdTime != null) {
    ...
    if (details.animation.playbackRate === 0) {
      ...
      details.startTime = timelineTime - details.holdTime / details.animation.playbackRate
      details.holdTime = null
    }
  } else if (details.startTime !== null && details.pendingPlaybackRate !== null) {
    ...
    const currentTimeToMatch = (timelineTime - details.startTime) * details.animation.playbackRate
    applyPendingPlaybackRate(details)
  ...
  if (details.readyPromise && details.readyPromise.state === 'pending')
    required(details.readyPromise).resolve(details.proxy)
...
function commitPendingPause(details: AnimationState): void {
  if (!details.timeline) return
  ...
  if (details.startTime != null && details.holdTime == null) {
    details.holdTime = (readyTime - details.startTime) * details.animation.playbackRate
  }
...
function commitFinishedNotification(details: AnimationState): void {
  if (details.finishedPromise?.state !== 'pending') return

  if (details.proxy.playState !== 'finished') return
  ...
  const finishedEvent = new CustomEvent('finish', {
    detail: {
      currentTime: details.proxy.currentTime,
      timelineTime: details.proxy.timeline?.currentTime ?? null,
    },
  })
  Object.defineProperty(finishedEvent, 'currentTime', {
    get: () => finishedEvent.detail.currentTime,
  })
  Object.defineProperty(finishedEvent, 'timelineTime', {
    get: () => finishedEvent.detail.timelineTime,
  })
...
function effectivePlaybackRate(details: AnimationState): number {
  if (details.pendingPlaybackRate !== null) return details.pendingPlaybackRate
  return details.animation.playbackRate
}

function applyPendingPlaybackRate(details: AnimationState): void {
 ...
 */
function silentlySetTheCurrentTime(
  details: AnimationState,
  seekTime: PolyfillNumberish | null,
): void {
  ...
  //     2.  Abort these steps.
  if (seekTime == null) {
    if (details.proxy.currentTime !== null) {
      throw new TypeError()
    }
    return
  }
  ...
  const seekTimeMs = required(fromCssNumberish(details, seekTime))
  ...
  if (
    ...
    details.holdTime = seekTimeMs
  } else {
    details.startTime =
      required(fromCssNumberish(details, required(details.timeline).currentTime)) -
      seekTimeMs / details.animation.playbackRate
  }
  ...
  if (!details.timeline || details.timeline.phase === 'inactive') {
    details.startTime = null
  }
...
function calculateCurrentTime(details: AnimationState): number | null {
  if (!details.timeline) return null

  const timelineTime = fromCssNumberish(details, details.timeline.currentTime)
  if (timelineTime === null) return null

  if (details.startTime === null) return null

  let currentTime = (timelineTime - details.startTime) * details.animation.playbackRate
...
function calculateStartTime(details: AnimationState, currentTime: number): number | null {
  if (!details.timeline) return null
...
function updateFinishedState(
  ...
  if (unconstrainedCurrentTime && details.startTime != null && !details.proxy.pending) {
    ...
    if (
      playbackRate > 0 &&
      unconstrainedCurrentTime >= upperBound &&
      details.previousCurrentTime != null
    ) {
      if (boundary === null || boundary < upperBound) boundary = upperBound
      details.holdTime = didSeek ? unconstrainedCurrentTime : boundary
    } else if (playbackRate < 0 && unconstrainedCurrentTime <= 0) {
      if (boundary == null || boundary > 0) boundary = 0
      details.holdTime = didSeek ? unconstrainedCurrentTime : boundary
  ...
  details.previousCurrentTime = fromCssNumberish(details, details.proxy.currentTime)

  // 4. Set the current finished state.
  const playState = details.proxy.playState

  if (playState === 'finished') {
    if (!details.finishedPromise) details.finishedPromise = new PromiseWrapper<ProxyAnimation>()
    ...
    if (details.finishedPromise && details.finishedPromise.state === 'resolved') {
      details.finishedPromise = new PromiseWrapper<ProxyAnimation>()
    }
    if (details.animation.playState !== 'paused') details.animation.pause()
  }
}

function effectEnd(details: AnimationState): number {
  // https://www.w3.org/TR/web-animations-1/#end-time
  const timing = normalizedTiming(details)
  const totalDuration =
    timing.delay +
    timing.endDelay +
    timing.iterations * (timing.duration === 'auto' ? 0 : timing.duration)

  return Math.max(0, totalDuration)
}

function _hasActiveTimeline(details: AnimationState): boolean {
  return details.timeline?.phase !== 'inactive'
}
...
function setNativeCurrentTime(details: AnimationState, time: number): void {
  const timeline = details.timeline
  const playbackRate = details.animation.playbackRate
  const atScrollTimelineBoundary =
    timeline?.currentTime && timeline.currentTime.value === (playbackRate < 0 ? 0 : 100)
  const delta = atScrollTimelineBoundary ? (playbackRate < 0 ? 0.001 : -0.001) : 0

  details.animation.currentTime = time + delta
}

function resetPendingTasks(details: AnimationState): void {
  ...
  //    "AbortError".
  required(details.readyPromise).reject(createAbortError())
...
function playInternal(details: AnimationState, _autoRewind: boolean): void {
  if (!details.timeline) return

  // https://drafts.csswg.org/web-animations/#playing-an-animation-section.
  // 1. Let aborted pause be a boolean flag that is true if animation has a
  //    pending pause task, and false otherwise.
  const abortedPause = details.proxy.playState === 'paused' && details.proxy.pending
  ...
  const previousCurrentTime = fromCssNumberish(details, details.proxy.currentTime)
  ...
  if (details.readyPromise && !hasPendingReadyPromise) details.readyPromise = null
  ...
  if (!details.readyPromise) createReadyPromise(details)
  details.pendingTask = 'play'
...
function tickAnimation(this: ProxyAnimation, timelineTime: PolyfillUnitValue | null): void {
...
function readTiming(effect: PolyfillAnimationEffect | null): NativeTiming {
  ...
  if (typeof duration !== 'number' && duration !== 'auto')
    throw new TypeError('Expected native millisecond timing')
  return {
    ...timing,
    duration,
...
function createProxyEffect(details: AnimationState): PolyfillAnimationEffect {
  ...
  const getTiming = (): NativeTiming => {
    ...
    if ((timing.duration === 'auto' || details.autoDurationEffect) && details.timeline) {
      details.autoDurationEffect = true
      timing.delay = timing.endDelay = 0
      const totalDuration = timing.iterations ? 100000 : 0
      timing.duration = timing.iterations ? totalDuration / timing.iterations : 0
      if (timing.duration < 0) {
        timing.duration = 0
        timing.endDelay = totalDuration
      }
      nativeUpdateTiming(timing)
    }
    details.normalizedTiming = timing
    return details.specifiedTiming
  }
  const getComputedTiming = (): PolyfillComputedTiming => {
    getTiming()
    const nativeTiming = effect.getComputedTiming()
    if (!details.timeline) return nativeTiming
    const timing = readTiming(effect)
    const rangeDuration = details.rangeDuration ?? 100
    const limit = effectEnd(details)
    const iterationDuration = timing.iterations
      ? (limit - timing.delay - timing.endDelay) / timing.iterations
      : 0
    return {
      ...nativeTiming,
      localTime:
        details.timeline.currentTime === null
          ? null
          : toCssNumberish(details, nativeNumber(nativeTiming.localTime)),
      endTime:
        toCssNumberish(details, nativeNumber(nativeTiming.endTime)) ?? numeric.CSS.percent(0),
      activeDuration:
        toCssNumberish(details, nativeNumber(nativeTiming.activeDuration)) ??
        numeric.CSS.percent(0),
      duration: numeric.CSS.percent(limit ? (rangeDuration * iterationDuration) / limit : 0),
    }
  }
  const updateTiming = (options?: OptionalEffectTiming): void => {
    if (details.timeline && options) {
      if (options.duration === Infinity || options.iterations === Infinity)
        throw new TypeError('Infinite scroll timing is unsupported')
      if (options.duration !== undefined && options.duration !== 'auto')
  ...
  const overrides: PolyfillAnimationEffect = { getTiming, getComputedTiming, updateTiming }
  return new Proxy<PolyfillAnimationEffect>(effect, {
    get(target, property) {
      if (
        property === 'getTiming' ||
        property === 'getComputedTiming' ||
...
function nativeNumber(value: CSSNumberish | null | undefined): number | null {
  if (value == null) return null
  if (typeof value !== 'number') throw new TypeError('Expected native millisecond time')
  return value
}

// Computes the start delay as a fraction of the active cover range.
function fractionalStartDelay(details: AnimationState): number {
  if (!details.animationRange || !details.timeline) return 0
  const rangeStart =
...
function fractionalEndDelay(details: AnimationState): number {
  if (!details.animationRange || !details.timeline) return 0
  const rangeEnd =
...
let proxyAnimations = new WeakMap<ProxyAnimation, AnimationState>()
export function getProxyDetails(proxy: ProxyAnimation): AnimationState {
  return required(proxyAnimations.get(proxy), 'Unregistered animation proxy')
}
...
export function installAnimationLifecycle(): void {
  window.addEventListener('pagehide', (event) => {
    if (!event.persisted) proxyAnimations = new WeakMap<ProxyAnimation, AnimationState>()
  })
  window.addEventListener('pageshow', (event) => {
    if (!event.persisted) return
    for (const animation of nativeDocumentGetAnimations.call(document)) {
      const proxy = proxiedAnimations.get(animation)
      const state = proxy && proxyAnimations.get(proxy)
      if (state?.timeline) updateMeasurements(state.timeline.source)
    }
  })
}

// Map from the real underlying native animation to the ProxyAnimation proxy of it.
const proxiedAnimations = new WeakMap<Animation, ProxyAnimation>()
export function getProxyForNativeAnimation(animation: Animation): ProxyAnimation | undefined {
  return proxiedAnimations.get(animation)
}
 ...
 * @param details
 */
function autoAlignStartTime(details: AnimationState): void {
  ...
  if (!details.timeline?.currentTime) {
    return
  }
  ...
  let startOffset: PolyfillUnitValue, endOffset: PolyfillUnitValue
  ...
  try {
    startOffset = numeric.CSS.percent(fractionalStartDelay(details) * 100)
  } catch (e) {
    ...
    startOffset = numeric.CSS.percent(0)
    if (details.animationRange) details.animationRange.start = 'normal'
  ...
  //    In the case of view timelines, it requires a calculation based on the proportion of the cover range.
  try {
    endOffset = numeric.CSS.percent((1 - fractionalEndDelay(details)) * 100)
  } catch (e) {
    // TODO: Validate supported values for range end, to avoid exceptions when resolving the values.

    // Range start is invalid, falling back to default value
    endOffset = numeric.CSS.percent(100)
    if (details.animationRange) details.animationRange.end = 'normal'
...
function unsupportedTimeline(_timeline: unknown): never {
  throw new Error('Unsupported timeline class')
}

function getNormalStartRange(timeline: ScrollTimeline): PolyfillNumericValue | ViewRangeOffset {
  return timeline instanceof ViewTimeline
    ? { rangeName: 'cover', offset: numeric.CSS.percent(0) }
    : numeric.CSS.percent(0)
}
function getNormalEndRange(timeline: ScrollTimeline): PolyfillNumericValue | ViewRangeOffset {
  return timeline instanceof ViewTimeline
    ? { rangeName: 'cover', offset: numeric.CSS.percent(100) }
    : numeric.CSS.percent(100)
}
export function parseAnimationRange(timeline: ScrollTimeline, value?: string): AnimationRange {
  if (!value) return { start: 'normal', end: 'normal' }
  if (timeline instanceof ViewTimeline) {
    const names: TimelineRangeName[] = []
    const offsets: PolyfillNumericValue[] = []
    for (const part of splitIntoComponentValues(value)) {
      if (isRangeName(part)) names.push(part)
      else offsets.push(numeric.CSSNumericValue.parse(part))
    }
    if (names.length > 2 || offsets.length > 2 || offsets.length === 1)
      throw new TypeError('Invalid or unsupported animation range')
    return {
      start: { rangeName: names[0] ?? 'cover', offset: offsets[0] ?? numeric.CSS.percent(0) },
      end: {
        rangeName: names[1] ?? names[0] ?? 'cover',
        offset: offsets[1] ?? numeric.CSS.percent(100),
      },
    }
  }
  if (!(timeline instanceof ScrollTimeline)) unsupportedTimeline(timeline)
  const parts = splitIntoComponentValues(value)
  if (parts.length !== 2) throw new TypeError('Invalid or unsupported animation range')
  return {
    start: numeric.CSSNumericValue.parse(required(parts[0])),
    end: numeric.CSSNumericValue.parse(required(parts[1])),
  }
}
function parseTimelineRangePart(
  timeline: ScrollTimeline,
  value: PolyfillRangeInput | undefined,
  position: 'start' | 'end',
): ResolvedRangePart {
  if (value == null || value === 'normal') return 'normal'
  if (value instanceof numeric.CSSKeywordValue)
    return parseTimelineRangePart(timeline, value.value, position)
  if (timeline instanceof ViewTimeline) {
    let rangeName: TimelineRangeName = 'cover'
    let offset: PolyfillNumericValue = numeric.CSS.percent(position === 'start' ? 0 : 100)
    if (value instanceof numeric.CSSNumericValue) {
      offset = value
    } else if (typeof value === 'object') {
      if ('rangeName' in value && value.rangeName !== undefined) rangeName = value.rangeName
      if ('offset' in value && value.offset !== undefined) offset = value.offset
    } else {
      const parts = splitIntoComponentValues(value)
      if (parts.length === 1) {
        const part = required(parts[0])
        if (isRangeName(part)) rangeName = part
        else offset = simplifyCalculation(numeric.CSSNumericValue.parse(part))
      } else if (parts.length === 2) {
        const name = required(parts[0])
        if (!isRangeName(name)) throw new TypeError('Invalid range name')
        rangeName = name
        offset = simplifyCalculation(numeric.CSSNumericValue.parse(required(parts[1])))
      } else throw new TypeError('Invalid range')
    }
    if (!isRangeName(rangeName)) throw new TypeError('Invalid range name')
    return { rangeName, offset }
  }
  if (!(timeline instanceof ScrollTimeline)) unsupportedTimeline(timeline)
  if (typeof value === 'object') {
    if ('rangeName' in value || 'offset' in value)
      throw new TypeError('Named ranges require a view timeline')
    if (!(value instanceof numeric.CSSNumericValue)) throw new TypeError('Invalid range offset')
    return value
  }
  return numeric.CSSNumericValue.parse(value)
}

// Create an alternate Animation class which proxies API requests.
// TODO: Create a full-fledged proxy so missing methods are automatically
// fetched from Animation.
export class ProxyAnimation implements PolyfillAnimation {
  constructor(
    effect: AnimationEffect | Animation | null = null,
    timeline?: AnimationTimeline | ScrollTimeline | null,
    animOptions: AnimationRelation = {},
  ) {
    const isScrollAnimation = timeline instanceof ScrollTimeline
    const animationTimeline = isScrollAnimation ? undefined : timeline
    const animation =
      effect instanceof nativeAnimation ? effect : new nativeAnimation(effect, animationTimeline)
    ...
    proxyAnimations.set(this, {
      ...
      animationRange: isScrollAnimation
        ? parseAnimationRange(timeline, animOptions['animation-range'])
        : null,
      proxy: this,
    })
  }
  ...
  get effect(): PolyfillAnimationEffect | null {
    const details = getProxyDetails(this)
    if (!details.timeline) return details.animation.effect

    // Proxy the effect to support timing conversions for progress based
    // animations.
    if (!details.effect) details.effect = createProxyEffect(details)

    return details.effect
  }
  set effect(newEffect: AnimationEffect | null) {
    const details = getProxyDetails(this)
  ...
  set timeline(newTimeline: AnimationTimeline | ScrollTimeline | null) {
    ...
    if (oldTimeline === newTimeline) return
    ...
    let previousProgress: number | null
    if (previousCurrentTime === null) {
      previousProgress = null
    } else if (end === 0) {
      previousProgress = 0
    } else {
      previousProgress = required(fromCssNumberish(details, previousCurrentTime)) / end
    }
    ...
    if (fromScrollTimeline) {
      if (details.timeline) removeAnimation(details.timeline, details.animation)
    }
    ...
    if (toScrollTimeline) {
      ...
      if (previousPlayState === 'running' || previousPlayState === 'finished') {
        ...
        // the scroll timeline.
        addAnimation(details.timeline, details.animation, tickAnimation.bind(this))
      }
      ...
      if (pending) {
        ...
        if (previousPlayState === 'paused') details.pendingTask = 'pause'
        else details.pendingTask = 'play'
      }
  ...
  get startTime(): PolyfillNumberish | null {
    const details = getProxyDetails(this)
    if (details.timeline) return toCssNumberish(details, details.startTime)

    return details.animation.startTime
  }
  set startTime(value: PolyfillNumberish | null) {
    ...
    const newStartTime = fromCssNumberish(details, value)
    if (!details.timeline) {
      details.animation.startTime = newStartTime
      return
    }
    ...
    if (details.startTime !== null && details.animation.playbackRate !== 0) details.holdTime = null
    else details.holdTime = previousCurrentTime
    ...
    if (details.pendingTask) {
      details.pendingTask = null
      required(details.readyPromise).resolve(this)
    }
  ...
  get currentTime(): PolyfillNumberish | null {
    const details = getProxyDetails(this)
    if (!details.timeline) return details.animation.currentTime

    if (details.holdTime != null) return toCssNumberish(details, details.holdTime)

    return toCssNumberish(details, calculateCurrentTime(details))
  }
  set currentTime(value: PolyfillNumberish | null) {
    const details = getProxyDetails(this)
    if (!details.timeline) {
      details.animation.currentTime = fromCssNumberish(details, value)
      return
    }
  ...
  get playbackRate(): number {
    return getProxyDetails(this).animation.playbackRate
  }
  set playbackRate(value: number) {
    ...
    //    previous time
    if (previousCurrentTime !== null) this.currentTime = previousCurrentTime
  }

  get playState(): AnimationPlayState {
    const details = getProxyDetails(this)
    if (!details.timeline) return details.animation.playState
    ...
    //      task,
    //    then idle.
    if (currentTime === null && details.startTime === null && details.pendingTask == null)
      return 'idle'
    ...
    //    then finished.
    if (currentTime != null) {
      if (details.animation.playbackRate > 0 && currentTime >= effectEnd(details)) return 'finished'
      if (details.animation.playbackRate < 0 && currentTime <= 0) return 'finished'
    }
  ...
  get rangeStart(): PolyfillRangeInput {
    return getProxyDetails(this).animationRange?.start ?? 'normal'
  }

  set rangeStart(value: PolyfillRangeInput) {
    const details = getProxyDetails(this)
    if (!details.timeline) {
      details.animation.rangeStart = value
      return
    }

    if (details.timeline instanceof ScrollTimeline) {
      details.animationRange ??= { start: 'normal', end: 'normal' }
  ...
  get rangeEnd(): PolyfillRangeInput {
    return getProxyDetails(this).animationRange?.end ?? 'normal'
  }

  set rangeEnd(value: PolyfillRangeInput) {
    const details = getProxyDetails(this)
    if (!details.timeline) {
      details.animation.rangeEnd = value
      return
    }
  ...
  get replaceState(): AnimationReplaceState {
    ...
    return getProxyDetails(this).animation.replaceState
  }
  ...
  finish(): void {
    ...
    if (details.startTime === null && timelineTime !== null) {
      details.startTime = timelineTime - limit / details.animation.playbackRate
    }
  ...
  pause(): void {
    ...
    if (details.pendingTask === 'play') details.pendingTask = null
    else details.readyPromise = null
  ...
  reverse(): void {
    ...
    if (!details.timeline || !reversable) {
      if (reversable) details.pendingPlaybackRate = -effectivePlaybackRate(details)
  ...
  updatePlaybackRate(rate: number): void {
    ...
    //    Abort these steps.
    if (details.readyPromise && details.readyPromise.state === 'pending') return
    ...
    switch (previousPlayState) {
      ...
      case 'finished': {
        ...
        const unconstrainedCurrentTime =
          timelineTime !== null && details.startTime !== null
        ...
        if (rate === 0) {
          ...
          details.startTime =
            timelineTime != null && unconstrainedCurrentTime != null
              ? (timelineTime - unconstrainedCurrentTime) / rate
              : null
        }
        applyPendingPlaybackRate(details)
        updateFinishedState(details, false, false)
  ...
  commitStyles(): void {
    getProxyDetails(this).animation.commitStyles()
  }

  persist(): void {
    getProxyDetails(this).animation.persist()
  }

  get id(): string {
    return getProxyDetails(this).animation.id
  }

  set id(value: string) {
    getProxyDetails(this).animation.id = value
  }
  ...
  get onfinish(): Animation['onfinish'] {
    return getProxyDetails(this).animation.onfinish
  }
  set onfinish(value: Animation['onfinish']) {
    getProxyDetails(this).animation.onfinish = value
  }
  get oncancel(): Animation['oncancel'] {
    return getProxyDetails(this).animation.oncancel
  }
  set oncancel(value: Animation['oncancel']) {
    getProxyDetails(this).animation.oncancel = value
  }
  get onremove(): Animation['onremove'] {
    return getProxyDetails(this).animation.onremove
  }
  set onremove(value: Animation['onremove']) {
    getProxyDetails(this).animation.onremove = value
  }

  get finished(): Promise<Animation | ProxyAnimation> {
    const details = getProxyDetails(this)
    if (!details.timeline) return details.animation.finished
  ...
  get ready(): Promise<Animation | ProxyAnimation> {
    const details = getProxyDetails(this)
    if (!details.timeline) return details.animation.ready
  ...
  addEventListener(
    ...
    if (!callback) return
    getProxyDetails(this).animation.addEventListener(type, callback, options)
  }
  ...
  removeEventListener(
    ...
    if (!callback) return
    getProxyDetails(this).animation.removeEventListener(type, callback, options)
  }

  dispatchEvent(event: Event): boolean {
    return getProxyDetails(this).animation.dispatchEvent(event)
  }
}
...
export function animate(
  ...
  options?: number | PolyfillAnimationOptions,
): ProxyAnimation {
  const settings = typeof options === 'number' ? { duration: options } : { ...options }
  const { timeline, rangeStart, rangeEnd, ...nativeOptions } = settings
  const nativeSettings: KeyframeAnimationOptions = nativeOptions
  if (timeline != null && !(timeline instanceof ScrollTimeline)) {
    // Only native timelines cross the native animate boundary.
    nativeSettings.timeline = timeline as AnimationTimeline
  } else if (timeline === null) nativeSettings.timeline = null
  const animation = nativeElementAnimate.call(this, keyframes, nativeSettings)
  const proxy = new ProxyAnimation(
    animation,
    timeline instanceof ScrollTimeline ? timeline : animation.timeline,
  )
  if (timeline instanceof ScrollTimeline) {
    animation.pause()
    getProxyDetails(proxy).animationRange = {
      start: parseTimelineRangePart(timeline, rangeStart, 'start'),
      end: parseTimelineRangePart(timeline, rangeEnd, 'end'),
    }
    proxy.play()
  }
  return proxy
}
function replaceProxiedAnimations(animations: Animation[]): (Animation | ProxyAnimation)[] {
  return animations.map((animation) => proxiedAnimations.get(animation) ?? animation)
}
export function elementGetAnimations(
  this: Element,
  options?: GetAnimationsOptions,
): (Animation | ProxyAnimation)[] {
  return replaceProxiedAnimations(nativeElementGetAnimations.call(this, options))
}
export function documentGetAnimations(this: Document): (Animation | ProxyAnimation)[] {
  return replaceProxiedAnimations(nativeDocumentGetAnimations.call(this))
}
