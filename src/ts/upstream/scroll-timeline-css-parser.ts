import type {
  ...
  AnonymousScrollOptions,
  AnonymousViewOptions,
  ScrollBindingOptions,
  ViewBindingOptions,
  KeyframeMapping,
} from './parser-types.js'
import { required } from '../platform/assertions.js'
import { TIMELINE_AXES, isTimelineAxis } from '../platform/constants.js'
import { ANIMATION_RANGE_NAMES, getAnonymousSourceElement } from './scroll-timeline-base.js'
...
export const TIMELINE_AXIS_TYPES = TIMELINE_AXES
const isAnonymousSource = (value: string): value is 'nearest' | 'root' | 'self' =>
  value === 'nearest' || value === 'root' || value === 'self'
...
export class StyleParser {
  cssRulesWithTimelineName: TimelineRule[]
  nextAnonymousTimelineNameIndex: number
  anonymousScrollTimelineOptions: Map<string, AnonymousScrollOptions | null>
  anonymousViewTimelineOptions: Map<string, AnonymousViewOptions | null>
  sourceSelectorToScrollTimeline: TimelineRegistration[]
  subjectSelectorToViewTimeline: TimelineRegistration[]
  keyframeNamesSelectors: Map<string, KeyframeMapping>
  ...
  transpileStyleSheet(sheetSrc: string, firstPass: boolean, srcUrl?: string): string {
    // AdhocParser
    const p = {
      sheetSrc: sheetSrc,
      index: 0,
    ...
    while (p.index < p.sheetSrc.length) {
      ...
      if (!rule) continue
      if (firstPass) this.parseKeyframesAndSaveNameMapping(rule, p)
      else this.handleScrollTimelineProps(rule, p)
    }
  ...
  getAnimationTimelineOptions(animationName: string, target: Element): AnimationRelation | null {
    ...
    for (let i = this.cssRulesWithTimelineName.length - 1; i >= 0; i--) {
      const current = required(this.cssRulesWithTimelineName[i])
      try {
  ...
  getAnonymousScrollTimelineOptions(
    timelineName: string,
    target: Element,
  ): ScrollBindingOptions | null {
  ...
  getScrollTimelineOptions(timelineName: string, target: Element): ScrollBindingOptions | null {
    const anonymousTimelineOptions = this.getAnonymousScrollTimelineOptions(timelineName, target)
    if (anonymousTimelineOptions) return anonymousTimelineOptions

    for (let i = this.sourceSelectorToScrollTimeline.length - 1; i >= 0; i--) {
      const options = required(this.sourceSelectorToScrollTimeline[i])
  ...
  findPreviousSiblingOrAncestorMatchingSelector(target: Element, selector: string): Element | null {
    // Target self
  ...
  getAnonymousViewTimelineOptions(
    timelineName: string,
    target: Element,
  ...
  getViewTimelineOptions(timelineName: string, target: Element): ViewBindingOptions | null {
    ...
    for (let i = this.subjectSelectorToViewTimeline.length - 1; i >= 0; i--) {
      const options = required(this.subjectSelectorToViewTimeline[i])
      if (options.name === timelineName) {
  ...
  handleScrollTimelineProps(rule: QualifiedRule, p: SheetCursor): void {
    ...
    let timelineNames: string[] = []
    let animationNames: string[] = []
    let shouldReplacePart = false

    if (hasAnimationTimeline) timelineNames = this.extractScrollTimelineNames(rule.block.contents)
    ...
    if (hasAnimation) {
      this.extractMatches(rule.block.contents, RegexMatcher.ANIMATION).forEach((shorthand) => {
        const animationName = this.extractAnimationName(shorthand)

        // Save this animation only if there is a scroll timeline.
        if (animationName && hasAnimationTimeline) animationNames.push(animationName)
        ...
        if (hasAnimationTimeline) {
          if (!this.hasDuration(shorthand)) {
            ...
            if (this.hasAutoDuration(shorthand)) {
              rule.block.contents = rule.block.contents.replace('auto', '    ')
            }
            ...
            rule.block.contents = rule.block.contents.replace(shorthand, ` 1s ${shorthand}`)
    ...
    if (shouldReplacePart) {
      this.replacePart(rule.block.startIndex, rule.block.endIndex, rule.block.contents, p)
    }

    this.saveRelationInList(rule, timelineNames, animationNames)
  }

  saveSourceSelectorToScrollTimeline(rule: QualifiedRule): void {
    ...
    const timelines: TimelineRegistration[] = []
    if (hasScrollTimeline) {
      ...
      for (const st of scrollTimelines) {
        ...
        const options: TimelineRegistration = { selector: rule.selector, name: '' }

        if (parts.length === 1) {
          options.name = required(parts[0])
        } else if (parts.length === 2) {
          if (isTimelineAxis(required(parts[0]))) {
            options.axis = parseAxis(required(parts[0]))
            options.name = required(parts[1])
          } else {
            options.axis = parseAxis(required(parts[1]))
            options.name = required(parts[0])
          }
        }
    ...
    if (hasScrollTimelineName) {
      ...
      for (let i = 0; i < names.length; i++) {
        if (i < timelines.length) {
          ...
          required(timelines[i]).name = required(names[i])
        } else {
          const options: TimelineRegistration = {
            selector: rule.selector,
            name: required(names[i]),
          }
          timelines.push(options)
        }
      }
    ...
    if (hasScrollTimelineAxis) {
      const extractedAxes = this.extractMatches(
        rule.block.contents,
        RegexMatcher.SCROLL_TIMELINE_AXIS,
      )
      axes = extractedAxes.filter(isTimelineAxis)
    ...
    for (let i = 0; i < timelines.length; i++) {
      if (axes.length) required(timelines[i]).axis = axes[i % axes.length]
    }

    this.sourceSelectorToScrollTimeline.push(...timelines)
  }

  saveSubjectSelectorToViewTimeline(rule: QualifiedRule): void {
    ...
    if (hasViewTimeline) {
      ...
      for (const tl of viewTimelines) {
        const parts = this.split(tl)
        const options: TimelineRegistration = { selector: rule.selector, name: '', inset: null }
        if (parts.length === 1) {
          options.name = required(parts[0])
        } else if (parts.length === 2) {
          if (isTimelineAxis(required(parts[0]))) {
    ...
    if (hasViewTimelineAxis) {
      const extractedAxes = this.extractMatches(
        rule.block.contents,
        RegexMatcher.VIEW_TIMELINE_AXIS,
      )
      axes = extractedAxes.filter(isTimelineAxis)
      if (axes.length !== extractedAxes.length) {
        throw new Error('Invalid axis')
    ...
    for (let i = 0; i < timelines.length; i++) {
      if (insets.length) required(timelines[i]).inset = insets[i % insets.length]
    ...
    for (let i = 0; i < maxLength; i++) {
      this.cssRulesWithTimelineName.push({
        ...
        ...(animationRanges.length
          ? { 'animation-range': animationRanges[i % animationRanges.length] }
          : {}),
      })
    }
  }

  extractScrollTimelineNames(contents: string): string[] {
    const value = required(required(RegexMatcher.ANIMATION_TIMELINE.exec(contents))[1]).trim()
  ...
  saveAnonymousTimelineName(part: string): string {
  ...
  parseAnonymousScrollTimeline(part: string): AnonymousScrollOptions | null {
    const anonymousMatch = RegexMatcher.ANONYMOUS_SCROLL_TIMELINE.exec(part)
    if (!anonymousMatch) return null

    const value = required(anonymousMatch[VALUES_CAPTURE_INDEX])
    const options: AnonymousScrollOptions = {}
    value.split(' ').forEach((token) => {
      if (isTimelineAxis(token)) {
        options.axis = token
      } else if (isAnonymousSource(token)) {
  ...
  parseAnonymousViewTimeline(part: string): AnonymousViewOptions | null {
  ...
  parseKeyframesAndSaveNameMapping(rule: QualifiedRule, p: SheetCursor): void {
    if (rule.selector.startsWith('@keyframes')) {
      ...
      rule.selector.split(' ').forEach((item, index) => {
        if (index > 0) this.keyframeNamesSelectors.set(item, mapping)
      })
    }
  }
  ...
  */
  replaceKeyframesAndGetMapping(rule: QualifiedRule, p: SheetCursor): KeyframeMapping {
    function hasPhase(selector: string): boolean {
      return ANIMATION_RANGE_NAMES.some((phase) => selector.startsWith(phase))
    }

    function cleanFrameSelector(selector: string): string {
      return selector
        .split(' ')
        .map((h) => h.trim())
        .filter((p) => p !== '')
    ...
    function getFrameSelectorIndexes(contents: string): { start: number; end: number }[] {
      let open = 0
      let startIndex = -1
      let endIndex = -1
      const indexes = []
    ...
    const mapping: KeyframeMapping = new Map()
    let foundPhaseLinkedOffset = false
    const newContents = []
    newContents.push(contents.substring(0, required(parts[0]).start))
    for (let i = 0; i < parts.length; i++) {
      const allFrameSelectors = contents.substring(required(parts[i]).start, required(parts[i]).end)
      const replacedFrameSelectors: string[] = []
      ...
      allFrameSelectors.split(',').forEach((currentFrameSelector) => {
        ...
        if (hasPhase(trimmedFrameSelector)) foundPhaseLinkedOffset = true
      })

      newContents.push(replacedFrameSelectors.join(','))

      if (i === parts.length - 1) newContents.push(contents.substring(required(parts[i]).end))
      else
        newContents.push(contents.substring(required(parts[i]).end, required(parts[i + 1]).start))
    }
  ...
  parseQualifiedRule(p: SheetCursor): QualifiedRule | undefined {
  ...
  removeEnclosingDoubleQuotes(s: string): string {
    const startIndex = s[0] === '"' ? 1 : 0
    const endIndex = s[s.length - 1] === '"' ? s.length - 1 : s.length
    return s.substring(startIndex, endIndex)
  }

  assertString(p: SheetCursor, s: string): void {
  ...
  replacePart(start: number, end: number, replacement: string, p: SheetCursor): void {
  ...
  eatComment(p: SheetCursor): void {
    this.assertString(p, '/*')
    this.eatUntil('*/', p, true)
    this.assertString(p, '*/')
  }

  eatBlock(p: SheetCursor): RuleBlock {
    const startIndex = p.index
    this.assertString(p, '{')
    let level = 1
    while (level !== 0) {
  ...
  parseError(p: SheetCursor, msg: string): Error {
    return Error(`(${p.name ? p.name : '<anonymous file>'}): ${msg}`)
  }

  eatUntil(s: string, p: SheetCursor, replaceWithSpace = false): string {
    const startIndex = p.index
  ...
  parseSelector(p: SheetCursor): string {
    const startIndex = p.index
    this.eatUntil('{', p)
  ...
  eatWhitespace(p: SheetCursor): void {
  ...
  lookAhead(s: string, p: SheetCursor): boolean {
    return p.sheetSrc.substr(p.index, s.length) === s
  }

  peek(p: SheetCursor): string | undefined {
    return p.sheetSrc[p.index]
  }

  extractMatches(contents: string, matcher: RegExp, separator = ','): string[] {
    const match = matcher.exec(contents)
    return match
      ? required(match[VALUES_CAPTURE_INDEX])
          .trim()
          .split(separator)
          .map((item) => item.trim())
      : []
...
function isAnonymousTimeline(part: string): boolean {
  return (part.startsWith('scroll') || part.startsWith('view')) && part.includes('(')
}

function isTime(s: string): RegExpExecArray | null {
  return RegexMatcher.TIME.exec(s)
}

function isNumber(s: string): RegExpExecArray | null {
  return RegexMatcher.NUMBER.exec(s)
}

export function removeKeywordsFromAnimationShorthand(anim: string): string[] {
  return anim
    .split(' ')
    .filter(
...
function parseAxis(value: string): ScrollAxis {
  if (!isTimelineAxis(value)) throw new TypeError('Invalid axis')
  return value
}
