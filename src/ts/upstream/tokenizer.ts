...
type CodePoint = number | undefined
type Lookahead = [CodePoint, CodePoint, CodePoint]
type NumericTokenType = 'integer' | 'number'
function fromCodePoints(...points: CodePoint[]): string {
  return String.fromCodePoint(
    ...points.map((point) => required(point, 'Unexpected end of code points')),
  )
}
...
export class IdentToken extends Token {
  override readonly kind = 'IdentToken' as const
  value: string
...
export class FunctionToken extends Token {
  override readonly kind = 'FunctionToken' as const
  value: string
  constructor(value: string) {
    super()
    this.value = value
...
export class AtKeywordToken extends Token {
  override readonly kind = 'AtKeywordToken' as const
  value: string
  constructor(value: string) {
    super()
    this.value = value
...
export class HashToken extends Token {
  override readonly kind = 'HashToken' as const
  type: 'id' | 'unrestricted'
  value: string
  constructor(value = '', type: 'id' | 'unrestricted' = 'unrestricted') {
    super()
    this.value = value
    this.type = type
  }
...
export class StringToken extends Token {
  override readonly kind = 'StringToken' as const
  value: string
  constructor(value: string) {
    super()
    this.value = value
...
export class BadStringToken extends Token {
  override readonly kind = 'BadStringToken' as const
}

export class UrlToken extends Token {
  override readonly kind = 'UrlToken' as const
  value: string
  constructor(value: string) {
    super()
    this.value = value
...
export class BadUrlToken extends Token {
  override readonly kind = 'BadUrlToken' as const
}

export class DelimToken extends Token {
  override readonly kind = 'DelimToken' as const
  value: string
  constructor(value: string) {
    super()
    this.value = value
...
export class NumberToken extends Token {
  override readonly kind = 'NumberToken' as const
  value: number
  type: NumericTokenType
  constructor(value: number, type: NumericTokenType = 'integer') {
    super()
    this.value = value
    this.type = type
  }
...
export class PercentageToken extends Token {
  override readonly kind = 'PercentageToken' as const
  value: number
...
export class DimensionToken extends Token {
  override readonly kind = 'DimensionToken' as const
  value: number
  type: NumericTokenType
  unit: string
  constructor(value: number, type: NumericTokenType, unit: string) {
    super()
    this.value = value
    this.type = type
    this.unit = unit
...
export class WhitespaceToken extends Token {
  override readonly kind = 'WhitespaceToken' as const
}

export class CDOToken extends Token {
  override readonly kind = 'CDOToken' as const
}

export class CDCToken extends Token {
  override readonly kind = 'CDCToken' as const
}

export class ColonToken extends Token {
  override readonly kind = 'ColonToken' as const
}

export class SemicolonToken extends Token {
  override readonly kind = 'SemicolonToken' as const
}

export class CommaToken extends Token {
  override readonly kind = 'CommaToken' as const
}

export class LeftSquareBracketToken extends Token {
  override readonly kind = 'LeftSquareBracketToken' as const
}

export class RightSquareBracketToken extends Token {
  override readonly kind = 'RightSquareBracketToken' as const
}

export class LeftParenthesisToken extends Token {
  override readonly kind = 'LeftParenthesisToken' as const
}

export class RightParenthesisToken extends Token {
  override readonly kind = 'RightParenthesisToken' as const
}

export class LeftCurlyBracketToken extends Token {
  override readonly kind = 'LeftCurlyBracketToken' as const
}

export class RightCurlyBracketToken extends Token {
  override readonly kind = 'RightCurlyBracketToken' as const
}
...
class InputStream {
  ...
  consume(): CodePoint {
    ...
    if (typeof codePoint !== 'undefined') {
      this.index += fromCodePoints(codePoint).length
    }
    return codePoint
  }

  reconsume(codePoint: CodePoint): void {
    if (typeof codePoint !== 'undefined') {
      this.index -= fromCodePoints(codePoint).length
    }
  }
  ...
  peek(): Lookahead {
    ...
    for (let i = 0; i < 3 && position < this.input.length; i++) {
      ...
      position += fromCodePoints(nextCodePoint).length
    }
    return [codePoints[0], codePoints[1], codePoints[2]]
  }
}

function isNewline(codePoint: CodePoint): boolean {
...
function isHexDigit(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
...
function isUppercaseLetter(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
...
function isLowercaseLetter(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
...
function isLetter(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
  // An uppercase letter or a lowercase letter.
  return isUppercaseLetter(codePoint) || isLowercaseLetter(codePoint)
}

function nonASCIICodePoint(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
  // A code point with a value equal to or greater than U+0080 <control>.
  return codePoint >= 0x0080
}
function isIdentStartCodePoint(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
  // A letter, a non-ASCII code point, or U+005F LOW LINE (_).
  return isLetter(codePoint) || nonASCIICodePoint(codePoint) || codePoint === 0x005f
}

function isIdentCodePoint(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
  // An ident-start code point, a digit, or U+002D HYPHEN-MINUS (-).
  return isIdentStartCodePoint(codePoint) || isDigit(codePoint) || codePoint === 0x002d
}

function isNonPrintableCodePoint(codePoint: CodePoint): boolean {
  if (codePoint === undefined) return false
...
function startsIdentSequence(
  ...
  if (firstCodePoint === 0x002d) {
    ...
    return (
      isIdentStartCodePoint(secondCodePoint) ||
      secondCodePoint === 0x002d ||
...
function consumeEscapedCodePoint(input: InputStream): number {
  ...
  if (isHexDigit(codePoint)) {
    const digits: CodePoint[] = [codePoint]
    // hex digit
  ...
  while (true) {
    ...
    if (codePoint === endingCodePoint) {
      ...
      if (typeof nextCodePoint === 'undefined') {
        ...
        stringToken.value += fromCodePoints(consumeEscapedCodePoint(input))
      }
    } else {
      // anything else
      // Append the current input code point to the <string-token>’s value.
      stringToken.value += fromCodePoints(codePoint)
    }
  }
}
 ...
 * @param {InputStream} input
 */
function consumeNumber(input: InputStream): { value: number; type: NumericTokenType } {
  // Execute the following steps in order:
  //
  // Initially set type to "integer". Let repr be the empty string.
  let type: NumericTokenType = 'integer'
  let repr = ''

  // If the next input code point is U+002B PLUS SIGN (+) or U+002D HYPHEN-MINUS (-), consume it and append it to repr.
  if ([0x002b, 0x002d].some((point) => point === input.peek()[0])) {
    repr += fromCodePoints(input.consume())
  }
  ...
  if (input.peek()[0] === 0x002e && isDigit(input.peek()[1])) {
    repr += fromCodePoints(input.consume(), input.consume())
    type = 'number'
    while (isDigit(input.peek()[0])) {
      repr += fromCodePoints(input.consume())
    }
  ...
  //   Set type to "number".
  //   While the next input code point is a digit, consume it and append it to repr.
  if ([0x0045, 0x0065].some((point) => point === input.peek()[0])) {
    if ([0x002d, 0x002b].some((point) => point === input.peek()[1]) && isDigit(input.peek()[2])) {
      repr += fromCodePoints(input.consume(), input.consume(), input.consume())
      type = 'number'
 ...
 * @param {InputStream} input
 */
function consumeNumericToken(input: InputStream): NumberToken | PercentageToken | DimensionToken {
 ...
 * @param {InputStream} input
 */
function consumeUrlToken(input: InputStream): UrlToken | BadUrlToken {
  ...
  while (true) {
    ...
    if (codePoint === 0x0029) {
      ...
      urlToken.value += fromCodePoints(codePoint)
    }
  }
}
...
function consumeIdentLikeToken(
  ...
  if (/^url$/i.test(str) && input.peek()[0] === 0x0028) {
    ...
    if (
      [0x0022, 0x0027].some((point) => point === input.peek()[0]) ||
      (isWhitespace(input.peek()[0]) && [0x0022, 0x0027].some((point) => point === input.peek()[1]))
    ) {
 ...
 * @param {InputStream} input
 */
function consumeToken(input: InputStream): CSSToken | undefined {
  // Consume the next input code point
  const codePoint = input.consume()
  ...
  if (isWhitespace(codePoint)) {
    ...
    if (isIdentCodePoint(lookahead[0]) || validEscape(lookahead[0], lookahead[1])) {
      ...
      return new DelimToken(fromCodePoints(codePoint))
    }
 ...
 * @param {string} str
 */
export function tokenizeString(str: string): CSSToken[] {
